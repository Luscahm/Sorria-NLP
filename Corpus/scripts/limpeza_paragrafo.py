...
#secoes = ["editorial", "descobrir", "conhecer", "cuidar", "conviver", "amar", "proteger", "movimentar", "comer", "brincar"]
# seções de 21 a 29:
...
#secao 38:
#secoes = ["prazeres simples", "COMO EU FAÇO?", "dá pra mudar?", "LUGAR EM COMUM", "VALORES ESSENCIAIS", "NOTA 10", "GENTE QUE FAZ", "AÇÃO E DIVERSÃO", "TÁ NA MESA", "como vai você?"]
#secao 39:
#secoes = ["carta da roberta", "prazeres simples", "COMO EU FAÇO?", "dá pra mudar?", "lugar em comum", "VALORES ESSENCIAIS", "NOTA 10", "GENTE QUE FAZ", "ação e diversão", "tá na mesa", "COMO VAI VOCÊ?"]
#secao 40:
#secoes = ["carta da roberta", "prazeres simples", "COMO EU FAÇO?", "dá pra mudar?", "LUGAR EM COMUM", "valores essenciais", "nota 10", "GENTE QUE FAZ", "ação e diversão", "tá na mesa", "COMO VAI VOCÊ?"]
#secao 41:
#secoes = ["carta da roberta", "prazeres simples", "como eu faço?", "dá pra mudar?", "valores essenciais", "nota 10", "gente que faz", "ação e diversão", "tá na mesa", "como vai você?"]
#secao 42:
#secoes = ["carta da roberta", "PraZeres simPles", "VALORES ESSENCIAIS", "Dá pra mudar?", "MaNuaL PrátIco", "geNte Que FaZ"]
#secoes 43 a 45; 47 a 51 ; 53 a ultima:
#secoes = ["CARTA DA ROBERTA","PRAZERES SIMPLES", "VALORES ESSENCIAIS", "DÁ PRA MUDAR?", "MANUAL PRÁTICO", "GENTE QUE FAZ"]
#secao 46:
#secoes = ["CaRta Da RoBeRta","PRAZERES SIMPLES", "VALORES ESSENcIAIS", "DÁ PRA MUDAR?", "ManuaL PRÁtiCo", "Gente Que FaZ"]
#secao 52:
...
#			continue
		...
		if edicao_data:
			...
			if meses is not False and  numero is not False and ano is not False:
				edicao_data = False
					...
					if len(coautor) == 0:
						...
						if len(autor) > 0:
							...
							if not(ignora) and len(linha.strip()) > 0:
								if re.search("\.\s", linha):
									paragrafo = linha.replace(".\n","$$")
									tira_aspas = paragrafo.strip().replace('"',"'")
...
saida = os.getcwd() + '/json_paragrafo/' + sys.argv[1].split("/")[-1].replace(".txt",".json")
